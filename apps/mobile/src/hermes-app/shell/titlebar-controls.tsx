...
import { PANE_TOGGLE_REVEAL_EVENT } from '@/components/pane-shell/pane-shell'
...
function toggleResponsivePaneReveal(paneId: string, dockedToggle: () => void) {
...
export function TitlebarControls({ leftTools = [], tools = [], onOpenSettings }: TitlebarControlsProps) {
  ...
  const fileBrowserEdge = { open: fileBrowserOpen, paneId: FILE_BROWSER_PANE_ID, toggle: toggleFileBrowserOpen }
  const sessionsEdge = { open: sidebarOpen, paneId: CHAT_SIDEBAR_PANE_ID, toggle: toggleSidebarOpen }
  ...
  const rightSidebarTool: TitlebarTool = {
    ...
    onSelect: () => {
      triggerHaptic('tap')
      toggleResponsivePaneReveal(rightEdge.paneId, rightEdge.toggle)
    }
  }
  ...
  return (
    <Tip label={tool.title ?? tool.label}>
      <Button
        ...
        onClick={() => {
          if (performance.now() - lastPointerSelectAt.current < 500) {
            return
          }
